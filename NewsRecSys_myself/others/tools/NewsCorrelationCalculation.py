"""
通过关键词，对每个类型下新闻的相似度计算
"""
import os
import sys
sys.path.append("../../")
from NewsRecSys_myself.settings import DB_HOST,DB_PORT,DB_USER,DB_PASSWD,DB_NAME
import pymysql
...
class Correlation:
    ...
    def loadData(self):
        ...
        for line in open(self.file, "r", encoding="utf-8").readlines():
            try:
                newid, newstags = line.strip().split("\t")
                news_tags[newid] = newstags
            except:
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        cor = Correlation(data_path+file)
        cor.writeToMySQL()
