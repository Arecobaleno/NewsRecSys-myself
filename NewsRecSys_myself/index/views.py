...
from NewsRecSys_myself.settings import ALLOW_USERS,ALLOW_TAGS
...
def home(request):
    ...
    total = 0  # 总页数
    # 如果cate是为你推荐，走该部分逻辑 tag_flag = 0 表示不是从标签召回数据
    if _cate == "1":
    ...
    result["cate_name"] = str(cate.objects.filter(cate_id=_cate))
    result["news"] = list()
    for one in news:
        print(one.new_id)
        result["news"].append({
            ...
            "new_hot_value": 0, #news_hot_value[one.new_id] if _cate == "2" or _cate == "1" else 0,
...
def getRecNews(request):
    ...
    # 走正常排序逻辑
        ...
        if newbrowse.objects.filter(user_name=uname).exists():
            ...
            #print(browse_dict)
            # 遍历最近浏览的N篇新闻，每篇新闻取 num 篇相似新闻
            for browse_one in browse_dict:
                obj = newsim.objects.filter(new_id_base=int(float(browse_one["new_id"]))).order_by(
                    "-new_correlation").values("new_id_sim")[:num]
                for one in obj:
                    news_id_list.append(float(one["new_id_sim"]))
                    # print((newhot.objects.filter(new_id=float(one["new_id_sim"]))))
                    all_news_hot_value[one["new_id_sim"]] = (newhot.objects.filter(new_id=float(one["new_id_sim"]))[0]).new_hot
            print(news_id_list)
            return new.objects.filter(new_id__in=news_id_list)[:20], all_news_hot_value
        else:
            # 如果该用户没用浏览记录，返回热度榜单的20-40
